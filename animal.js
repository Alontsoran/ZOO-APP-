function renderAnimal(animal) {
  ...
  document.getElementById(
    "image"
  ).innerHTML = `<img src="${animal.image}" alt="${animal.name}" style="width:280px; margin: 10px;   max-width: 90%;" />`;
}
...
function feedAnimal(animal, visitor) {
  ...
  if (parseInt(visitor.coins) >= 2) {
    ...
    const animal_ = visitor.Documentation.find(
      (animal_) => animal_.name === animal.name
    ); //הולך לחיה המתאימה
...
function openDialog(text) {
  ...
  document
    .getElementById("Logout_from_user")
...
function getvisitorHTMLCard(animal) {
  const template = `
    <div id="card-${
      animal.name
    }" class="cardss" style="width:200px; margin: 10px;">
...
function FilterByLivingEnvironment(animalsInzoo) {
  ...
  const animals = animalsInzoo.filter(
    (animal_) => animal_.habitat === animal.habitat
  );
  return animals;
}
